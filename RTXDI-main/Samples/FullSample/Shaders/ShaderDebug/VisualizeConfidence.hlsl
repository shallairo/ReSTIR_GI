...
float4 main(float4 i_position : SV_Position) : SV_Target
{
    ...
    float input = 0;
    if (g_Const.visualizationMode == VISUALIZATION_OVERLAY_MODE_DIFFUSE_CONFIDENCE)
        input = t_DiffuseConfidence[inputPos];
    else if (g_Const.visualizationMode == VISUALIZATION_OVERLAY_MODE_SPECULAR_CONFIDENCE)
